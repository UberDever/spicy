...
void init(void) {
    ...
    trianglelist_new(&list, 4, 12);
    v3 vert[4] = {{0, 0, 0}, {0.3, 0, 0}, {.3, 1.1, 0}, {0, 1.1, 0}};
    v3 initial_offset; pi_v3(initial_offset, -0.15f, -0.55f, 0);
    uint ind[12] = {0, 1, 3, 3, 1, 2, 3, 1, 0, 2, 1, 3};
    v2 tex[4] = {{ 0, 0 }, { 1, 0 }, { 1, 1 }, {0, 1}};
    for (uint i = 0; i < ARR_ONE_DIM_LEN(vert); i++)
    {
        pi_v3_add(vert[i], initial_offset, vert[i]);
